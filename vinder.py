...
def _setup_facebook_cookies():
    ...
    try:
        import tempfile
        fd, path = tempfile.mkstemp(prefix='vinder_fb_cookies_', suffix='.txt')
        with os.fdopen(fd, 'w') as f:
            f.write(cookies_content)
...
def _setup_instagram_cookies():
    ...
    try:
        import tempfile
        fd, path = tempfile.mkstemp(prefix='vinder_ig_cookies_', suffix='.txt')
        with os.fdopen(fd, 'w') as f:
            f.write(cookies_content)
    ...
    try:
        ...
        if is_tiktok:
            ...
            if is_fb:
                ...
                _ydl_opts_ig_fast = {'format': 'bestaudio/best', 'quiet': True, 'no_warnings': True, 'noplaylist': True}
...
def _fb_get_info(url):
    """
    Ambil metadata video Facebook via yt-dlp (primary) + FB cookies.
    Fallback ke facebook-scraper kalau yt-dlp gagal.
    Support: /reel/, /watch?v=, /videos/, /share/v/, fb.watch
    """
    url = _fb_resolve_url(url)
    logger.info(f"[FB] Ambil info via yt-dlp: {mask_url(url)}")

    # --- PRIMARY: yt-dlp + FB cookies ---
    try:
        ydl_opts = {
            'quiet':       True,
            'no_warnings': True,
        ...
        logger.warning(f"[FB] yt-dlp gagal ({type(e).__name__}: {e}), coba fallback facebook-scraper...")

    # --- FALLBACK: facebook-scraper (hanya jalan untuk post/watch biasa, bukan Reels) ---
    try:
        from facebook_scraper import get_posts
        logger.info(f"[FB] Ambil info via facebook-scraper (fallback): {mask_url(url)}")
        for post in get_posts(post_urls=[url], options={"posts_per_page": 1}):
            video_url = post.get('video') or post.get('video_url') or post.get('video_id')
            title     = post.get('text', '') or post.get('post_text', '') or 'Facebook Video'
            title     = title.replace('\n', ' ')[:80]
            cover     = post.get('image') or post.get('thumbnail') or ''
            author    = post.get('username') or post.get('user_id') or 'Facebook'
            dur_raw   = post.get('video_duration_seconds') or post.get('duration') or 0
            try:
                dur_sec = int(dur_raw)
            except Exception:
                dur_sec = 0
            logger.info(f"[FB] facebook-scraper OK: title={title[:40]} | video_url={'Ada' if video_url else 'Kosong'}")
            return {
                'title':        title,
                'cover':        cover,
                'author':       author,
        ...
        raise RuntimeError("facebook-scraper tidak mengembalikan data apapun untuk URL ini.")
    except Exception as e:
        logger.error(f"[FB] _fb_get_info semua strategi gagal: {type(e).__name__}: {e}")
        raise
...
def download_mp4_api():
    ...
    try:
        if is_ig:
            ...
            # ── FACEBOOK: download via facebook-scraper ──
            is_fb = any(x in url for x in ['facebook.com', 'fb.watch'])
            if not is_fb:
                return jsonify({"status": "error", "msg": "Platform tidak didukung."}), 400
...
HEALTH_SAMPLES = {
    "TikTok":    "https://vt.tiktok.com/ZS9GBdy9y/",
    "Instagram": "https://www.instagram.com/reel/DYWhyBfhACA/?igsh=N3V5dHNuaDNocWtz",
    "Facebook":  "https://www.facebook.com/share/r/14dcfuwdywn/",
}
...
def _run_daily_health_check():
    ...
    # --- Instagram: test via _ig_get_info_instaloader ---
    ig_error = None
    try:
        ig_info = _ig_get_info_instaloader(HEALTH_SAMPLES['Instagram'])
        if not ig_info or not ig_info.get('is_video'):
            ig_error = f"instaloader berhasil konek tapi konten bukan video atau info kosong. Data: {str(ig_info)[:200]}"
        else:
    ...
    if ig_error:
        ...
        platform_results.append("[Instagram] ✅ OK — instaloader berhasil ambil metadata video.")
...
def _heartbeat_cookies_loop():
    """
    Kirim request ringan ke Instagram, Facebook, YouTube setiap 8 jam
    ...
    while True:
        ...
        if _COOKIES_FILE and os.path.exists(_COOKIES_FILE):
            try:
                ...
                with yt_dlp.YoutubeDL(_ydl_opts_yt_hb) as ydl:
                    ydl.extract_info(_HB_SAMPLES['YouTube'], download=False)
                logger.info("[HEARTBEAT] YouTube cookies — session OK.")
                hasil.append("▶️ YouTube ✅ Session aktif")
            except Exception as e:
                logger.warning(f"[HEARTBEAT] YouTube cookies heartbeat gagal: {e}")
                hasil.append(f"▶️ YouTube ❌ Gagal: {e}")
        else:
            logger.info("[HEARTBEAT] YouTube cookies tidak ada, skip.")
