...
def mp4_info_api():
    ...
    try:
        ...
        logger.error(f"[MP4INFO] Error yt-dlp: {e}")
        # Fallback ke RapidAPI YTStream khusus untuk YouTube
        is_yt = any(x in url for x in ['youtube.com', 'youtu.be'])
        if is_yt:
            logger.info("[MP4INFO] yt-dlp gagal, mencoba fallback RapidAPI YTStream...")
            yt_info = _ytstream_get_info(url)
            if yt_info:
                best_fmt = yt_info['formats'][0] if yt_info['formats'] else {}
                return jsonify({
                    "status":   "success",
                    "title":    yt_info['title'],
                    "cover":    yt_info['thumbnail'],
                    "author":   "YouTube",
                    "duration": format_durasi(yt_info['duration']),
                    "size":     "N/A",
                    "play":     best_fmt.get('url', url),
                    "hdplay":   best_fmt.get('url', url),
                })
            logger.error("[MP4INFO] RapidAPI YTStream juga gagal.")
...
def download_mp4_api():
    ...
    try:
        ...
        logger.error(f"[MP4DL] Error yt-dlp: {e}")
        # Fallback ke RapidAPI YTStream khusus untuk YouTube
        is_yt = any(x in url for x in ['youtube.com', 'youtu.be'])
        if is_yt:
            logger.info("[MP4DL] yt-dlp gagal, mencoba fallback RapidAPI YTStream...")
            yt_info = _ytstream_get_info(url)
            if yt_info and yt_info['formats']:
                # Pilih format terbaik sesuai quality request
                formats = yt_info['formats']
                if quality != 'best':
                    low = [f for f in formats if '480' in f.get('quality', '') or '360' in f.get('quality', '')]
                    formats = low if low else formats
                best = formats[0]
                dl_url   = best['url']
                filename = f"[Vinder].{safe_filename(title)}.mp4"
                logger.info(f"[MP4DL] YTStream fallback berhasil, stream dari CDN URL")
                r_yt = session.get(dl_url, stream=True, timeout=30, headers=DEFAULT_HEADERS)
                if r_yt.status_code == 200:
                    return Response(
                        stream_with_context(r_yt.iter_content(chunk_size=1024 * 1024)),
                        headers={
                            'Content-Type':        'video/mp4',
                            'Content-Disposition': make_content_disposition(filename),
                            'Cache-Control':       'no-cache',
            ...
            logger.error("[MP4DL] RapidAPI YTStream juga gagal.")
