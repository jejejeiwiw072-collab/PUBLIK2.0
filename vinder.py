...
def _fb_get_info(url):
    ...
    try:
        ...
        # LOG DETAIL: dump semua format untuk analisis
        formats = info.get('formats') or []
        logger.info(f"[FB-DEBUG] Total formats tersedia: {len(formats)}")
        for i, f in enumerate(formats):
            logger.info(
                f"[FB-DEBUG] fmt[{i}] id={f.get('format_id')} | "
                f"ext={f.get('ext')} | "
                ...
                f"filesize_approx={f.get('filesize_approx')}"
            )
