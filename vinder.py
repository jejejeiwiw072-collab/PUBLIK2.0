...
def search_videos_api():
    ...
    limit      = min(data.get('limit', 15), 15)
...
def download_mp4_api():
    ...
    try:
        if is_ig:
            ...
            # FIX: Prioritaskan format H.264 (avc1) dulu untuk kompatibilitas galeri HP
            # Kalau tidak ada H.264, fallback ke best lalu re-encode via ffmpeg ke H.264
            if quality == 'best':
                fmt = (
            ...
            ydl_opts = {
                ...
                # FIX: Re-encode ke H.264/AAC kalau format yang didapat bukan H.264
                # Ini jalan otomatis hanya saat ffmpeg perlu transcode (AV1/VP9/etc)
                'postprocessors': [{
