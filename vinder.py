...
def mp4_info_api():
    ...
    try:
        ...
        if is_ig:
            ...
            # PROBE 1: tanpa format filter, dengan player_client & proxy saat ini
            ydl_opts_probe = {**ydl_opts, 'format': 'best'}
            try:
                with yt_dlp.YoutubeDL(ydl_opts_probe) as ydl_probe:
                    info_probe = ydl_probe.extract_info(url, download=False)
                    formats_probe = info_probe.get('formats') or []
                    logger.info(f"[MP4INFO][PROBE1] Total format tersedia: {len(formats_probe)}")
                    for f in formats_probe:
                        logger.info(f"[MP4INFO][PROBE1] id={f.get('format_id')} ext={f.get('ext')} vcodec={f.get('vcodec')} acodec={f.get('acodec')} height={f.get('height')} abr={f.get('abr')}")
            except Exception as probe_err:
                logger.warning(f"[MP4INFO][PROBE1] Gagal: {probe_err}")
            # PROBE 2: tanpa extractor_args dan tanpa proxy — isolasi apakah player_client/proxy yang salah
            ydl_opts_probe2 = {
                'format':      'best',
                'quiet':       True,
                'no_warnings': True,
            ...
            if _COOKIES_FILE and os.path.exists(_COOKIES_FILE):
                ydl_opts_probe2['cookiefile'] = _COOKIES_FILE
            try:
                with yt_dlp.YoutubeDL(ydl_opts_probe2) as ydl_probe2:
                    info_probe2 = ydl_probe2.extract_info(url, download=False)
                    formats_probe2 = info_probe2.get('formats') or []
                    logger.info(f"[MP4INFO][PROBE2] Total format tersedia (no proxy, no client): {len(formats_probe2)}")
                    for f in formats_probe2:
                        logger.info(f"[MP4INFO][PROBE2] id={f.get('format_id')} ext={f.get('ext')} vcodec={f.get('vcodec')} acodec={f.get('acodec')} height={f.get('height')} abr={f.get('abr')}")
            except Exception as probe_err2:
                logger.warning(f"[MP4INFO][PROBE2] Gagal juga (no proxy, no client): {probe_err2}")
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=False)
        ...
        err_str = str(e)
        logger.error(f"[MP4INFO] Error: {err_str}")
        logger.error(f"[MP4INFO] URL yang gagal: {mask_url(url)}")
        logger.error(f"[MP4INFO] is_ig={is_ig if 'is_ig' in dir() else 'N/A'}")
        logger.error(f"[MP4INFO] Format string dipakai: {ydl_opts.get('format') if 'ydl_opts' in dir() else 'N/A'}")
        logger.error(f"[MP4INFO] player_client dipakai: {ydl_opts.get('extractor_args') if 'ydl_opts' in dir() else 'N/A'}")
        logger.error(f"[MP4INFO] proxy dipakai: {ydl_opts.get('proxy') if 'ydl_opts' in dir() else 'N/A'}")
        logger.error(f"[MP4INFO] cookies aktif: {bool(_COOKIES_FILE and os.path.exists(_COOKIES_FILE))}")
...
def download_mp4_api():
    ...
    try:
        if is_ig:
            ...
            if _COOKIES_FILE and os.path.exists(_COOKIES_FILE):
                ydl_opts['cookiefile'] = _COOKIES_FILE
                logger.info("[MP4DL] yt-dlp pakai cookies YouTube.")
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download([url])
