...
def mp4_info_api():
    ...
    try:
        ...
        if is_ig:
            ...
            # PROBE: dump semua format tersedia tanpa filter — untuk diagnosis error format
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts_probe) as ydl_probe:
                    ...
                    logger.info(f"[MP4INFO][PROBE] Total format tersedia: {len(formats_probe)}")
                    for f in formats_probe:
                        logger.info(f"[MP4INFO][PROBE] id={f.get('format_id')} ext={f.get('ext')} vcodec={f.get('vcodec')} acodec={f.get('acodec')} height={f.get('height')} abr={f.get('abr')}")
            except Exception as probe_err:
                logger.warning(f"[MP4INFO][PROBE] Probe juga gagal: {probe_err}")
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=False)
