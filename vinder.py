...
def mp4_info_api():
    ...
    try:
        ...
        logger.error(f"[MP4INFO] yt-dlp version: {yt_dlp.version.__version__}")
        logger.error(f"[MP4INFO] URL: {mask_url(url)}")
        logger.error(f"[MP4INFO] Exception type: {type(e).__name__}")
        logger.error(f"[MP4INFO] Full traceback:\n{traceback.format_exc()}")
