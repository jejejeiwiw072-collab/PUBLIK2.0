...
def download_audio_ytdlp(url, out_mp3):
    ...
    Untuk YouTube: prioritas VDA API kalau VDA_API_KEY tersedia, fallback ke yt-dlp.
    """
    is_youtube = any(x in url for x in ['youtube.com', 'youtu.be'])

    if is_youtube and _VDA_API_KEY:
        logger.info(f"[VDA] YouTube terdeteksi + VDA_API_KEY ada, pakai VDA API.")
        try:
            _vda_download_audio(url, out_mp3)
            return
        except Exception as e:
            logger.warning(f"[VDA] VDA gagal, fallback ke yt-dlp: {e}")
...
def _vda_download_audio(url, out_mp3):
    """
    Download audio via VDA API (p.savenow.to).
    Flow: GET job -> poll status jika async -> download file ke out_mp3.
    Dipakai untuk YouTube sebagai pengganti yt-dlp langsung.
    Return True kalau berhasil, raise RuntimeError kalau gagal.
    """
    if not _VDA_API_KEY:
        raise RuntimeError("VDA_API_KEY tidak ditemukan di env. Set env var VDA_API_KEY.")

    VDA_ENDPOINT = "https://p.savenow.to/ajax/download.php"

    logger.info(f"[VDA] Submit job: {mask_url(url)}")
    resp = requests.get(
        VDA_ENDPOINT,
        params={"format": "mp3", "url": url, "apikey": _VDA_API_KEY},
        timeout=30
    )
    resp.raise_for_status()
    data = resp.json()
    logger.info(f"[VDA] Response awal: {str(data)[:200]}")
    ...
    # Jika job async — ada job_id / id untuk polling
    job_id = data.get("job_id") or data.get("id") or data.get("task_id")

    if not download_url and job_id:
        logger.info(f"[VDA] Job async terdeteksi, polling job_id={job_id}")
        for attempt in range(1, 31):  # max 30x polling = ~60 detik
            time.sleep(2)
            poll = requests.get(
                VDA_ENDPOINT,
                params={"job_id": job_id, "apikey": _VDA_API_KEY},
                timeout=15
            )
            poll.raise_for_status()
            poll_data = poll.json()
            logger.info(f"[VDA] Poll #{attempt}: {str(poll_data)[:150]}")

            status = poll_data.get("status", "").lower()
            download_url = (
                poll_data.get("download_url") or
                poll_data.get("url") or
                poll_data.get("link") or
                poll_data.get("file")
            )

            if download_url:
                logger.info(f"[VDA] Download URL siap setelah {attempt} polling.")
                break

            if status in ("error", "failed", "fail"):
                raise RuntimeError(f"VDA job gagal: {poll_data.get('message', 'unknown error')}")

    if not download_url:
        raise RuntimeError(f"VDA tidak mengembalikan download URL. Response: {str(data)[:300]}")

    logger.info(f"[VDA] Download file dari: {mask_url(download_url)}")
    ...
    size_mb = os.path.getsize(out_mp3) / 1024 / 1024
    logger.info(f"[VDA] Download selesai: {size_mb:.2f} MB -> {out_mp3}")
    return True


def _vda_get_info(url):
    """
    Ambil metadata video YouTube via VDA API tanpa download file.
    Return dict: {title, cover, author, duration, size} atau raise RuntimeError.
    """
    if not _VDA_API_KEY:
        raise RuntimeError("VDA_API_KEY tidak ditemukan di env.")

    VDA_ENDPOINT = "https://p.savenow.to/ajax/download.php"

    logger.info(f"[VDA] Get info: {mask_url(url)}")
    resp = requests.get(
        VDA_ENDPOINT,
        params={"format": "info", "url": url, "apikey": _VDA_API_KEY},
        timeout=20
    )
    resp.raise_for_status()
    data = resp.json()
    logger.info(f"[VDA] Info response: {str(data)[:300]}")

    # Ekstrak field dari response VDA
    title    = data.get("title") or data.get("name") or "YouTube Video"
    cover    = data.get("thumbnail") or data.get("thumb") or data.get("cover") or ""
    author   = data.get("author") or data.get("channel") or data.get("uploader") or "Unknown"
    duration = data.get("duration") or data.get("length") or 0
    size_raw = data.get("filesize") or data.get("size") or 0
    size_str = f"{int(size_raw) / 1024 / 1024:.2f}MB" if size_raw else "N/A"

    # duration bisa dalam detik (int) atau string "mm:ss"
    if isinstance(duration, int):
        dur_str = format_durasi(duration)
    else:
...
def _vda_download_mp4(url, out_mp4, quality="best"):
    """
    Download video MP4 YouTube via VDA API.
    quality: 'best' -> format=mp4, selainnya -> format=mp4_sd
    Flow: GET job -> poll jika async -> download file ke out_mp4.
    """
    if not _VDA_API_KEY:
        raise RuntimeError("VDA_API_KEY tidak ditemukan di env.")

    VDA_ENDPOINT = "https://p.savenow.to/ajax/download.php"
    fmt = "mp4" if quality == "best" else "mp4_sd"

    logger.info(f"[VDA] Submit MP4 job (quality={quality}): {mask_url(url)}")
    resp = requests.get(
        VDA_ENDPOINT,
        params={"format": fmt, "url": url, "apikey": _VDA_API_KEY},
        timeout=30
    )
    resp.raise_for_status()
    data = resp.json()
    logger.info(f"[VDA] MP4 response awal: {str(data)[:200]}")
    ...
    if not download_url and job_id:
        logger.info(f"[VDA] MP4 job async, polling job_id={job_id}")
        for attempt in range(1, 31):
            ...
            poll.raise_for_status()
            poll_data = poll.json()
            logger.info(f"[VDA] MP4 Poll #{attempt}: {str(poll_data)[:150]}")
            ...
            if download_url:
                logger.info(f"[VDA] MP4 URL siap setelah {attempt} polling.")
                break

            if status in ("error", "failed", "fail"):
                raise RuntimeError(f"VDA MP4 job gagal: {poll_data.get('message', 'unknown error')}")

    if not download_url:
        raise RuntimeError(f"VDA tidak mengembalikan MP4 URL. Response: {str(data)[:300]}")

    logger.info(f"[VDA] Download MP4 dari: {mask_url(download_url)}")
    ...
    logger.info(f"[VDA] MP4 download selesai: {size_mb:.2f} MB -> {out_mp4}")
    return True
...
_VDA_API_KEY     = os.environ.get('VDA_API_KEY', '')
...
def download_url_api():
    ...
    try:
        ...
        is_youtube = any(x in url_input for x in ['youtube.com', 'youtu.be'])
        if is_youtube and _VDA_API_KEY:
            logger.info(f"[VDA] download_url_api: pakai VDA untuk YouTube info.")
            info = _vda_get_info(url_input)
            return jsonify({
                "status":   "success",
                "title":    info["title"],
                "cover":    info["cover"],
...
def fast_mp3_api():
    ...
    try:
        ...
        if is_tiktok:
            ...
            # Non-TikTok (YouTube, Instagram, Facebook): pakai VDA untuk YouTube, yt-dlp untuk lainnya
            ...
            is_youtube = any(x in tiktok_url for x in ['youtube.com', 'youtu.be'])

            if is_youtube and _VDA_API_KEY:
                logger.info(f"[VDA] fast_mp3: pakai VDA untuk YouTube.")
                info_vda    = _vda_get_info(tiktok_url)
                final_title = info_vda["title"]
                _vda_download_audio(tiktok_url, out_mp3_yt)
            else:
...
def mp4_info_api():
    ...
    try:
        ...
        if is_ig:
            ...
            if is_youtube and _VDA_API_KEY:
                logger.info(f"[VDA] mp4_info_api: pakai VDA untuk YouTube info.")
                info = _vda_get_info(url)
                return jsonify({
                    "status":   "success",
                    "title":    info["title"],
                    "cover":    info["cover"],
            ...
            if _COOKIES_FILE and os.path.exists(_COOKIES_FILE):
                ydl_opts['cookiefile'] = _COOKIES_FILE
                logger.info("[MP4INFO] yt-dlp pakai cookies YouTube.")
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=False)
...
def download_mp4_api():
    ...
    try:
        if is_ig:
            ...
            # ── YOUTUBE / FACEBOOK: download via VDA untuk YouTube, yt-dlp untuk lainnya ──
            ...
            if is_youtube and _VDA_API_KEY:
                logger.info(f"[VDA] download_mp4_api: pakai VDA untuk YouTube.")
                _vda_download_mp4(url, out_mp4, quality=quality)
            else:
