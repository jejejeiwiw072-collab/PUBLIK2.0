...
VDA_DOWNLOAD_ENDPOINT = "https://p.savenow.to/ajax/download.php"


def _vda_clean_youtube_url(url):
    """
    Bersihkan URL YouTube dari tracking params (?si=, &pp=, dll).
    Shorts: https://youtube.com/shorts/ID?si=xxx -> https://www.youtube.com/watch?v=ID
    Watch:  https://youtu.be/ID?si=xxx           -> https://www.youtube.com/watch?v=ID
    """
    ...
    try:
        ...
        if '/shorts/' in parsed.path:
            ...
            return f"https://www.youtube.com/watch?v={video_id}"
        # youtube.com/watch?v=ID — buang tracking params, pertahankan v saja
        ...
        logger.warning(f"[VDA] Gagal clean URL: {e}")
    return url


def _vda_submit_and_poll(fmt, url):
    """
    Submit job ke VDA API dan poll progress_url sampai download_url tersedia.
    fmt: 'mp3' atau 'mp4'
    Return: download_url string, raise RuntimeError kalau gagal.

    Submit response: {success: true, id: "...", title: "...", info: {image: "..."}, progress_url: "..."}
    Progress response: {success: 0, progress: 0-1000, download_url: ""}
    File siap ketika download_url tidak kosong.
    """
    if not _VDA_API_KEY:
        raise RuntimeError("VDA_API_KEY tidak ditemukan di env. Set env var VDA_API_KEY.")

    url = _vda_clean_youtube_url(url)
    logger.info(f"[VDA] Submit job format={fmt}: {mask_url(url)}")
    resp = requests.get(
        VDA_DOWNLOAD_ENDPOINT,
        params={"format": fmt, "url": url, "apikey": _VDA_API_KEY},
        timeout=30
    )
    resp.raise_for_status()
    data = resp.json()
    logger.info(f"[VDA] Submit response: {str(data)[:300]}")

    if not data.get("success"):
        raise RuntimeError(f"VDA submit gagal: {str(data)[:200]}")

    progress_url = data.get("progress_url")
    if not progress_url:
        job_id = data.get("id")
        if not job_id:
            raise RuntimeError(f"VDA tidak mengembalikan id atau progress_url. Response: {str(data)[:200]}")
        progress_url = f"https://p.savenow.to/api/progress?id={job_id}"

    logger.info(f"[VDA] Polling: {progress_url}")
    for attempt in range(1, 91):  # max 90x polling = ~3 menit
        time.sleep(2)
        poll = requests.get(progress_url, timeout=15)
        poll.raise_for_status()
        poll_data    = poll.json()
        progress     = poll_data.get("progress", 0)
        download_url = poll_data.get("download_url", "")
        logger.info(f"[VDA] Poll #{attempt} progress={progress} url={'ada' if download_url else 'kosong'}")

        if download_url:
            logger.info(f"[VDA] File siap setelah {attempt} polling.")
            return download_url

    raise RuntimeError("VDA timeout: file tidak siap setelah 3 menit.")


def _vda_download_audio(url, out_mp3):
    """
    Download audio MP3 YouTube via VDA API.
    Submit job format=mp3 -> poll /ajax/progress -> download file ke out_mp3.
    """
    download_url = _vda_submit_and_poll("mp3", url)

    logger.info(f"[VDA] Download MP3 dari: {mask_url(download_url)}")
    dl_resp = requests.get(download_url, stream=True, timeout=180)
    ...
    size_mb = os.path.getsize(out_mp3) / 1024 / 1024
    logger.info(f"[VDA] MP3 download selesai: {size_mb:.2f} MB -> {out_mp3}")
    return True
...
def _vda_download_mp4(url, out_mp4, quality="best"):
    ...
    Submit job format=mp4 -> poll /ajax/progress -> download file ke out_mp4.
    """
    fmt = "mp4" if quality == "best" else "mp4_sd"
    download_url = _vda_submit_and_poll(fmt, url)
...
def fast_mp3_api():
    ...
    try:
        ...
        if is_tiktok:
            ...
            # Non-TikTok (YouTube, Instagram, Facebook)
            ...
            if is_youtube and _VDA_API_KEY:
                # YouTube: pakai VDA API langsung — tidak perlu ambil info dulu
                logger.info(f"[VDA] fast_mp3: YouTube -> VDA API.")
                _vda_download_audio(tiktok_url, out_mp3_yt)
                final_title = title  # title sudah dikirim dari frontend
            else:
...
def mp4_info_api():
    ...
    try:
        ...
        if is_ig:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=False)
                # Ambil info format terbaik untuk di-log saja (tidak re-fetch)
                formats = info.get('formats') or []
                if formats:
                    # Cari format dengan video+audio sekaligus (bukan adaptive stream)
                    combined = [f for f in formats if f.get('vcodec') not in (None, 'none') and f.get('acodec') not in (None, 'none')]
                    if combined:
                        best_fmt = max(combined, key=lambda f: f.get('height') or 0)
                    else:
                        best_fmt = max(formats, key=lambda f: f.get('height') or 0)
                    logger.info(f"[MP4INFO] Format tersedia: {best_fmt.get('format_id')} (height={best_fmt.get('height')})")
...
def download_mp4_api():
    ...
    try:
        if is_ig:
            ...
            # ── YOUTUBE: VDA API | FACEBOOK/LAINNYA: yt-dlp ──
            ...
            if is_youtube and _VDA_API_KEY:
                logger.info(f"[VDA] download_mp4: YouTube -> VDA API (quality={quality}).")
