...
def mp4_info_api():
    ...
    try:
        ...
        if is_ig:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=False)
                # Pilih format terbaik dari daftar format yang tersedia
                formats = info.get('formats') or []
                if formats:
                    best_fmt = max(formats, key=lambda f: f.get('height') or 0)
                    best_format_id = best_fmt.get('format_id')
                    logger.info(f"[MP4INFO] Format terpilih: {best_format_id} (height={best_fmt.get('height')})")
