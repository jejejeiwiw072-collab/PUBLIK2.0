...
# =============================================================================
# VERSI FILE — JANGAN HAPUS, BUAT TRACKING
# Build : v3 — 2026-05-11
# Fix   : Pass 1 - format: 'best' + enable DASH manifest (comment youtube_include_dash_manifest)
#         Pass 2 - format: 'bestvideo+bestaudio/best' agar ada fallback
#         Root cause: DASH manifest dimatikan = format audio Shorts tidak terbaca
# =============================================================================
_VINDER_BUILD = "v3-20260511"
...
logger.info(f"[BUILD] Vinder build: {_VINDER_BUILD}")
...
def mp4_info_api():
    ...
    # Strip tracking params (?si=, ?igshid=) sebelum hit yt-dlp — bikin extract_info inconsistent
    from urllib.parse import urlparse, urlencode, parse_qs, urlunparse
    _parsed = urlparse(url)
    _clean_qs = {k: v for k, v in parse_qs(_parsed.query).items() if k not in ('si', 'igshid', 'igsh')}
    url = urlunparse(_parsed._replace(query=urlencode(_clean_qs, doseq=True)))
    ...
    try:
        ...
        if is_ig:
            ...
            ydl_opts_meta = {
                ...
                'extract_flat':                  False,
                'skip_download':                 True,
                'format':                        'best',  # FIX v3: cukup 'best' karena hanya butuh metadata
                # 'youtube_include_dash_manifest': False, # FIX v3: di-comment agar semua format terbaca
            }
            if _YT_COOKIES_PATH:
            ...
            try:
                ydl_opts_size = {
                    ...
                    'noplaylist':    True,
                    'skip_download': True,
                    'format':        'bestvideo+bestaudio/best',  # FIX v3: fallback '/best' agar tidak error
                }
                if _YT_COOKIES_PATH:
