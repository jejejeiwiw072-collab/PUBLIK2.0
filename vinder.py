...
def _fb_get_info(url):
    """
    Ambil metadata video Facebook via facebook-scraper.
    Return dict {title, cover, author, duration_sec, video_url} atau raise Exception.
    URL /share/v/ di-resolve dulu ke URL asli sebelum dilempar ke get_posts.
    """
    from facebook_scraper import get_posts

    url = _fb_resolve_url(url)
    logger.info(f"[FB] Ambil info via facebook-scraper: {mask_url(url)}")
    try:
        for post in get_posts(post_urls=[url], options={"posts_per_page": 1}):
            ...
            try:
                ...
                dur_sec = 0
            logger.info(f"[FB] Info OK: title={title[:40]} | video_url={'Ada' if video_url else 'Kosong'}")
            return {
                'title':        title,
                'cover':        cover,
                'author':       author,
...
def _fb_download_audio(url, out_mp3):
    """
    Download audio Facebook via facebook-scraper + ffmpeg encode ke MP3.
    Ambil video_url, pipe ke ffmpeg langsung (sama seperti download_audio_direct).
    """
    info = _fb_get_info(url)
    video_url = info.get('video_url')
    if not video_url:
