...
def _fb_get_info(url):
    ...
    try:
        ...
        # Facebook pakai DASH: video & audio stream terpisah, acodec selalu 'none'
        # Filter hanya berdasarkan height, target sweet spot <= 720p
        video_url = None
        ...
        if video_formats:
            ...
            logger.info(f"[FB] Sweet spot format: {best.get('height')}p | tbr={best.get('tbr')} | vcodec={best.get('vcodec')}")
        else:
            # Fallback: ambil format video dengan height terendah yang tersedia
            any_video = [
                f for f in formats
                if f.get('url') and (f.get('height') or 0) > 0 and f.get('vcodec') not in (None, 'none')
            ]
            if any_video:
                best = min(any_video, key=lambda f: f.get('height') or 9999)
                video_url = best.get('url')
                logger.info(f"[FB] Fallback format terendah: {best.get('height')}p | tbr={best.get('tbr')}")
            else:
                video_url = info.get('url') or ((info.get('formats') or [{}])[-1].get('url'))
                logger.info(f"[FB] Tidak ada format video terdeteksi, fallback ke url default")
...
def _fb_download_video(url, out_mp4):
    """
    Download video Facebook ke out_mp4 via facebook-scraper.
    Ambil video_url dari post, lalu download langsung via requests streaming.
    """
    info = _fb_get_info(url)
    video_url = info.get('video_url')
    if not video_url:
        raise RuntimeError("URL video Facebook tidak ditemukan oleh facebook-scraper.")

    logger.info(f"[FB] Download video dari: {mask_url(video_url)}")
    headers = DEFAULT_HEADERS.copy()
