...
def _fb_get_info(url):
    ...
    try:
        ...
        # Pilih format sweet spot: MP4 resolusi <= 1080p, prioritas tertinggi mendekati 1080p
        video_url = None
        ...
        if mp4_formats:
            ...
            logger.info(f"[FB] Sweet spot format: {best.get('height')}p | ext={best.get('ext')}")
        else:
            video_url = info.get('url') or (formats[0].get('url') if formats else None)
            logger.info(f"[FB] Tidak ada format MP4 <=1080p, fallback ke url default")
